...
export default class ChannelList extends Component {
  ...
  filterById (id) {
    return this.state.messages.filter(e => e.channelId===id)
  }
  render () {
      ...
      <ul>
        <li>
          <NavLink to={RANDOM_CHANNEL} activeClassName="active">
            ...
            <span className="badge">{this.filterById(+RANDOM_CHANNEL.slice(-1)).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={GENERAL_CHANNEL} activeClassName="active">
            <span># generally_speaking</span>
            <span className="badge">{this.filterById(+GENERAL_CHANNEL.slice(-1)).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={DOGS_CHANNEL} activeClassName="active">
            <span># dogs_of_fullstack</span>
            <span className="badge">{this.filterById(+DOGS_CHANNEL.slice(-1)).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={LUNCH_CHANNEL} activeClassName="active">
            <span># lunch_planning</span>
            <span className="badge">{this.filterById(+LUNCH_CHANNEL.slice(-1)).length}</span>
          </NavLink>
        </li>
      </ul>
    );
